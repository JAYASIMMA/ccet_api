...
const StaffModel = __importStar(require("../models/technicalStaff"));
const JWT_SECRET = process.env.JWT_SECRET || 'secretString';
// Auth
const register = async (req, res, next) => {
    try {
        const { password, ...data } = req.body;
        const existing = await StaffModel.getTechnicalStaffByEmail(data.email);
        if (existing)
            return res.status(400).json({ message: 'Email already exists' });
        const hashedPassword = await bcryptjs_1.default.hash(password, 10);
        await StaffModel.createTechnicalStaff({ ...data, password: hashedPassword });
        res.status(201).json({ message: 'Technical Staff registered successfully' });
    }
    catch (error) {
        next(error);
    }
...
const login = async (req, res, next) => {
    try {
        ...
        const user = await StaffModel.getTechnicalStaffByEmail(email);
        if (!user)
            return res.status(404).json({ message: 'User not found' });
        const isMatch = await bcryptjs_1.default.compare(password, user.password);
        if (!isMatch)
            return res.status(400).json({ message: 'Invalid credentials' });
        const token = jsonwebtoken_1.default.sign({ id: user.id, role: 'technical-staff', email: user.email }, JWT_SECRET, { expiresIn: '1h' });
        res.json({ token, user: { id: user.id, name: user.name, email: user.email, role: 'technical-staff' } });
    }
    catch (error) {
        next(error);
    }
